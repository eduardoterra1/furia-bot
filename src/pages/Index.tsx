...
const Index = () => {
  ...
  return <div className="min-h-screen bg-gradient-to-b from-furia-primary via-furia-primary to-black p-4 md:p-8">
      <div className="max-w-6xl mx-auto">
        {/* Header */}
        <div className="flex justify-center items-center mb-8">
          <h1 className="font-gaming text-4xl md:text-5xl furia-text-gradient animate-pulse-furia">FURIA.GG</h1>
        </div>
        ...
        <Tabs defaultValue="chat" className="w-full">
              ...
              <CardContent className="p-4">
                <div className="flex flex-col h-[calc(100vh-250px)]">
                  ...
                  <form onSubmit={handleSendMessage} className="flex gap-2">
                    <Input placeholder="Digite sua mensagem..." value={chatInput} onChange={e => setChatInput(e.target.value)} className="bg-furia-primary/60 border-furia-secondary text-white" />
                    <Button type="submit" disabled={loading} className="bg-furia-secondary hover:bg-furia-secondary/90">
                      Enviar
                    </Button>
                  </form>
                </div>
          ...
          <TabsContent value="matches" className="mt-2">
            <div className="grid gap-6">
              ...
              {matchStatuses.map((match, index) => <MatchStatusCard key={index} {...match} />)}
            </div>
          </TabsContent>
          ...
          <TabsContent value="players" className="mt-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {furiaPlayers.map((player, index) => <PlayerCard key={index} {...player} />)}
            </div>
          </TabsContent>
          
          {/* Trivia Tab */}
          <TabsContent value="trivia" className="mt-2">
            <TriviaQuestion {...triviaQuestions[currentQuestionIndex]} onNextQuestion={() => setCurrentQuestionIndex(prev => (prev + 1) % triviaQuestions.length)} />
          </TabsContent>
